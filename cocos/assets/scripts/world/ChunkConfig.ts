...
export class ChunkItemGenerator {
  public static generateChunkItems(
    ...
    for (let i = 0; i < count; i++) {
      // 保证前几个分块中至少 75% 为 T1 基础可吸附物，25% 为高 Tier 目标 (用于测试 Tier Lock)
      ...
      const localX = Math.max(-14.0, Math.min(14.0, clusterBaseX + jitterX));
      
      // Z 轴均匀错开
      const localZ = -halfLen + 3.0 + ((i * 1.35) % (chunkLength - 6.0));
