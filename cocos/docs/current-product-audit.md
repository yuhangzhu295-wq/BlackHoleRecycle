...
- The verified gameplay camera is now 44° vertical FOV with a 20m/18.5m follow offset; the 390×844 runtime report measures the player at 19.9% viewport width (inside the 18–23% readability band). On wide desktop previews the native Cocos camera viewport is letterboxed to the same centred portrait rectangle, so 3D terrain no longer spills into the side bars.
...
- The complete five-level machine path is live rather than configuration-only. The latest paid-skin touch-only Web Mobile run reached LV2 in Bedroom, LV3 in Warehouse (compression chamber visible), LV4 in Supermarket (gravity wing visible), LV5 in Parking (singularity frame visible), then absorbed a Creator-rendered T5 city sedan. It recorded a final mass of 851205 kg and T1–T5 absorption counts from the ordinary compression system before the skin purchase.
...
- The vertical slice is real at runtime: LV1 starts at T1/2.4m, visibly locks T2, absorbs T1 to evolve, reaches LV2/3.4m, then absorbs T2. Latest full acceptance evidence recorded LV2, mass 3125, 30 T1 and 5 T2 absorbed.
- `Game.scene` contains Creator-saved portrait Home, Mode Select, Skin Selection, gameplay HUD, Arena HUD, Revive, Pause, Settlement, and Machine Info page nodes. Their actions drive live state and real save data rather than placeholder buttons. The mode page's return control and title were re-authored inside the portrait safe area so the actual mobile frame no longer clips or overlaps them.
- The Home skin action now opens the Creator-saved Skin Selection page instead of cycling a hidden list. Both starter cores can be equipped and persisted by touch; paid cores validate against `SKINS_CONFIG`, atomically deduct only earned coins, and leave the save unchanged with an explicit “金币不足” result when unaffordable. The save normalizer now uses explicit array de-duplication instead of `Set` spread, avoiding Cocos' loose WebGL transform that previously serialized ownership as `[{}]`; paid unlock, equip, and coin debit are now one persisted transaction.
...
- `npm run acceptance:v2 -- --scope=skin-unlock`: an official Cocos Web Mobile build and complete real-touch route. It earns coins through the full LV1→LV5 gameplay route (including live T5 city absorption), pauses through the saved page, returns Home, opens the saved Skin Selection page, and buys/equips the orange core. Latest evidence: `287012 → 285512` coins, selected `skin_orange_force`, browser console errors `[]`, no acceptance failures.
...
- `arena-server/` has a separate Colyseus integration test that connects two actual SDK clients to one server-authoritative room and observes replicated movement, pickup suction, LV2/T2 progression, defeat, dropped recyclable mass, respawn, a server-generated match id, and a server-finalized FORFEIT reward ledger. The Cocos Web client can make a real browser WebSocket connection to that room, forwards its actual visible joystick at 20Hz, and maps the replicated ledger into the saved Settlement page. The client persists the match id before applying coins, so a page reload cannot claim the same probe result twice; production account storage must still enforce this server-side. The network acceptance route proves a real player session, server input-sequence advancement, server-replicated position change, and the visible finish/settlement path with browser console errors `[]`; it uses no state setter or mock room.

## Deliberate boundaries / remaining work

1. **Online arena is not product-complete.** The server owns multiplayer joining, bot fill, input sequencing, stale-input timeout, position integration, pickup suction, LV1-to-LV2 progression, combat, dropped fragments, respawn, match-end reason, and the final per-player reward ledger. The Cocos opt-in browser path consumes server snapshots through `NetworkArenaReplica`, renders remote machines/pickups, forwards actual joystick input, and applies the replicated reward once to the visible Settlement page with a persisted match-id guard. Server-side account anti-replay storage, a public `wss://` endpoint, production matchmaking entry, and WeChat/ByteDance device evidence remain required before the game may advertise human matchmaking.
...
- `PAID_SKIN_UNLOCK_GATE`: PASS
- `ONLINE_REWARD_LEDGER_GATE`: PASS (local Colyseus authority + Cocos probe settlement path)
