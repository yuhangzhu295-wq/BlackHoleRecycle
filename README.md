...
- `arena-server/` 包含经双客户端集成测试的 Colyseus 本地权威房间（输入序号、位置、吸收、LV2、T2、淘汰、质量碎片、复活和服务器结算奖励）。Cocos Web Mobile 提供显式 `?arenaProbe=<endpoint>` 联机探针，真实渲染服务器快照、转发摇杆并进入结算页；默认“竞技吞噬”仍是本地 1+7。由于尚无公网 `wss://`、账号级防重放存储和生产匹配入口，产品仍不宣称在线匹配。
